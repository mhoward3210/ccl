...
J = @(q)ccl_rob_jacobian(q,settings.link.length);                       % robot jacobian
...
%     settings.lambda = lambda;
    ...
    model.c     = ccl_math_gc (Xtr, model.num_basis) ;      % generate a grid of basis functions
    model.s2    = mean(mean(sqrt(ccl_math_distances(model.c, model.c))))^2 ;   % set the variance as the mean distance between centres
    model.phi   = @(x)ccl_basis_rbf ( x, model.c, model.s2 );      % normalised Gaussian rbfs
    
    % learn the nullspace component
    model       = ccl_learnv_ncl (Xtr, Ytr, model) ;                       % learn the model
    f_ncl       = @(x) ccl_learnv_pred_ncl ( model, x ) ;                     % set up an inference function
    ...
    % calculate errors
    NUPEtr = ccl_error_nupe(NStr, NSptr) ;
    NUPEte = ccl_error_nupe(NSte, NSpte) ;
    YNSptr = ccl_error_npe (Ytr,  NSptr) ;
    YNSpte = ccl_error_npe (Yte,  NSpte) ;
...
for i = 1:settings.dim_exp
    ...
    elseif strcmp(settings.projection, 'state_dependant')
        settings.lambda = @(q)ccl_learna_retlambda(q,settings.link.length);
    ...
    model_alpha_ccl  = ccl_learna_alpha (NStr, Xtr, settings);
    ppe.alpha_ccl.tr = ccl_error_ppe_alpha (model_alpha_ccl.f_proj, Xtr, Ftr, NStr) ;
    ppe.alpha_ccl.te = ccl_error_ppe_alpha (model_alpha_ccl.f_proj, Xte, Fte, NSte) ;
    poe.alpha_ccl.tr = ccl_error_poe_alpha (model_alpha_ccl.f_proj, Xtr, NStr) ;
    poe.alpha_ccl.te = ccl_error_poe_alpha (model_alpha_ccl.f_proj, Xte, NSte) ;
    ...
    model_lambda_ccl  = ccl_learna_lambda (NStr, Xtr, J, settings);
    ppe.lambda_ccl.tr = ccl_error_ppe_alpha (model_lambda_ccl.f_proj, Xtr, Ftr, NStr) ;
    ppe.lambda_ccl.te = ccl_error_ppe_alpha (model_lambda_ccl.f_proj, Xte, Fte, NSte) ;
    poe.lambda_ccl.tr = ccl_error_poe_alpha (model_lambda_ccl.f_proj, Xtr, NStr) ;
    poe.lambda_ccl.te = ccl_error_poe_alpha (model_lambda_ccl.f_proj, Xte, NSte) ;
    ...
    ccl_rob_vis_sbs (settings,model_lambda_ccl.f_proj)
...
settings.learn_pi       = 1  ;
if strcmp(settings.projection, 'state_independant')
    settings.A = @(q)(orth( rand(2, 1) )' * ccl_rob_jacobian (q, settings.link.length));
elseif strcmp(settings.projection, 'state_dependant')
    lambda = @(q)ccl_learna_retlambda(q,settings.link.length,2);
    settings.A = @(q)(lambda(q) * ccl_rob_jacobian (q, settings.link.length));
end
...
for i = 1:settings.dim_exp
    ...
    % set up the radial basis functions
    model.c     = ccl_math_gc (Xtr, model.dim_b) ;      % generate a grid of basis functions
    model.s2    = mean(mean(sqrt(ccl_math_distances(model.c, model.c))))^2 ;   % set the variance as the mean distance between centres
    model.phi   = @(x)ccl_basis_rbf ( x, model.c, model.s2 );      % normalised Gaussian rbfs
    
    % train parametric model
    model = ccl_learnp_pi(Xtr,NStr,model); fp = @(x)ccl_learnp_pred_linear(x,model);
    
    % predict training data
    Fptr = fp(Xtr);
    
    % compute training error
    NUPEtr = ccl_error_nupe(Ftr,Fptr);
    fprintf(1,'NUPE (train) = %5.3e, ',NUPEtr);
    NCPEtr = ccl_error_ncpe(NStr,Fptr,Ptr);
    ...
    % compute test error
    NUPEte = ccl_error_nupe(Fte,Fpte);
    fprintf(1,'NUPE (test) = %5.3e, ',NUPEte);
    NCPEte = ccl_error_ncpe(NSte,Fpte,Pte);
...
for i = 1:settings.dim_exp
    ...
    model.W   = @(x)ccl_basis_rbf( x, model.c, model.s2 );
    model.phi = @(x)ccl_basis_linear( x );
    %model.phi = @(x)phi_gaussian_rbf ( x, model.c, model.s2 );
    
    % train the model
    model = ccl_learnp_lwpi(Xtr,NStr,model); fp = @(x)ccl_learnp_pred_lwlinear(x,model);
