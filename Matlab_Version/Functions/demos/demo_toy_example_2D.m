...
settings.null_policy_type = 'linear_attractor';                 % {'limit_cycle' 'linear_attractor' 'linear'}
...
if strcmp(settings.projection,'state_independant')
    ...
    for i = 1:settings.dim_exp
        ...
        model  = ccl_learna_nhat (NStr);
        
        fprintf(1,'True projection:\n w =\n'),      disp(P);
        fprintf(1,'Estimated projection:\n wp =\n'),disp(model.P)
        
        % make prediction
        fp   = @(f)ccl_learna_pred_proj (f,model);
        Yptr = fp(Ftr);
        Ypte = fp(Fte);
        Ypv  = fp(Fv) ;
        ...
        fprintf(1,'\t       |    NPPE   |   NPOE \n' ) ;
        fprintf(1,'\t -------------------------------\n' ) ;
        nPPE = ccl_error_ppe(Ytr, model.P, Ftr) ;
        nPOE = ccl_error_poe(Ytr, model.P, Ftr) ;
        fprintf(1,'\t Train |  %4.2e | %4.2e  \n', nPPE,  nPOE ) ;
        nPPE = ccl_error_ppe(Yte, model.P, Fte) ;
        nPOE = ccl_error_poe(Yte, model.P, Fte) ;
...
s2   = 0.5;
model.phi = @(x)ccl_basis_rbf ( x, c, s2 );
...
s2 = 0.005;
model = [];
model.W   = @(x)ccl_basis_rbf( x, c, s2 );
