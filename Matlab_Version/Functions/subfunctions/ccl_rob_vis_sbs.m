...
elseif strcmp(settings.projection,'state_dependant')
    a = 2;
    nhat = @(x)([2*a.*[1,0]*ccl_rob_forward(x,settings.link.length), -1]) ;
end

for n = 1 : settings.dim_n+1
    J   = ccl_rob_jacobian (x, L) ;
    r   = ccl_rob_forward(x, L) ;       r_   = ccl_rob_forward(x1, L) ;
    A   = nhat(x) * J ;
    invA= pinv(A) ;
    P_  = Iu - invA*A;
...
figh = figure;
ccl_rob_vis_move (figh,R, X, L,'Movement under true constraint',[500,200,500,500]) ;
figh = figure;
ccl_rob_vis_move (figh,R_, X_, L,'Movement under Learnt constraint',[1000,200,500,500]) ;
end