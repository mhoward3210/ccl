function data = generate_data_ccl (settings)
% data = generate_alpha_ccl (settings)
%
...
if settings.grid_on == 1
    ...
    for k = 1: settings.nTraj
        ...
        if isfield(settings,'learn_lambda')
            ...
            if strcmp(settings.projection, 'state_independant')
                generate_A = @(q)(settings.lambda * jacobian (q, L));
            elseif strcmp(settings.projection, 'state_dependant')
                generate_A = @(q)(settings.lambda(q) * jacobian (q, L));
            end
        end
        ...
        if settings.learn_nc == 1
            ...
            generate_A = @(q)(f_lambda * jacobian (q, L));
        end
